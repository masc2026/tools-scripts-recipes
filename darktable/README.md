...
Das ist eine sinnvolle Änderung, da Darktable mit dem Feld "CSS-Anpassungen" mittlerweile einen sehr bequemen Weg bietet, UI-Tweaks persistent zu speichern, ohne ganze Theme-Dateien warten zu müssen.

**Zu deiner Frage (Repo-Datei vs. README):**
Ich empfehle, den CSS-Block **in eine Datei im Repo zu legen** (z. B. `tweaks.css` oder der alte Name `darktable_masc.css`), aber den Inhalt **zusätzlich** im README (evtl. in einem ausklappbaren Bereich) anzuzeigen.

* **Grund:** Wenn du den Code nur im README hast, verlierst du Syntax-Highlighting und die Möglichkeit, Änderungen am CSS sauber per Git zu tracken. Liegt er in einer Datei, kannst du die Datei einfach im Editor öffnen, `Strg+A` / `Strg+C` machen und in Darktable einfügen.

Hier ist der Vorschlag für die aktualisierte `README.md` unter Berücksichtigung deiner Formulierungs-Vorgaben (kurz, kein Genitiv, passiv).

### Vorschlag für README.md
...
Inhalt der Datei [`darktable_masc.css`](darktable_masc.css) (oder untenstehenden Block) kopieren und in Darktable einfügen unter:
**Einstellungen > Allgemeines > CSS-Anpassungen**
